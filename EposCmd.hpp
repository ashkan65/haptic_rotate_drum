...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	else
	{
		...
		if(VCS_SetPositionProfile(p_DeviceHandle, p_usNodeId, pProfileVelocity, pProfileAcceleration, pProfileDeceleration, &p_rlErrorCode) == 0)
		{
...
bool DemoProfileVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	msg << "skip profile velocity mode, node = " << p_usNodeId;
		...
		if(lResult==0)
		{
			...
			if(lResult==0)
			{
				if(!oIsEnabled)
				{
					...
					char pErrorInfo[1000];
					VCS_GetErrorInfo(*p_pErrorCode, pErrorInfo, 1000);
					LogInfo(pErrorInfo);
				}
			}
		}
